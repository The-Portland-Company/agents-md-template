...
Client-agnostic operating manual. Any agent runtime (Claude Code, Codex, OpenClaw, etc.) reads this. Runtime-specific mechanics live in the sibling `Agents.md` (Claude Code) — this file holds the principles and rules that apply everywhere.
...
- **Precedence:** for principles, this file governs. For runtime mechanics (skills, task tools, notifications, exact question format), the runtime file (`Agents.md`) governs and overrides any generic phrasing here.
...
- When you must ask, use the runtime's question format (Claude Code: see `Agents.md` → HITL). Present options with their key pros/cons.
...
# Agents.md — Claude Code Specifics
